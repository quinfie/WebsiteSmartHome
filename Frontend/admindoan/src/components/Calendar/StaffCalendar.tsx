...
const StaffCalendar: React.FC<StaffCalendarProps> = ({ events }) => {
    ...
    const getEventColor = (event: UserEvent) => {
        const status = event.status.toLowerCase();
        if (status.includes('hoàn thành')) {
            return 'bg-green-100 text-green-800 border-green-200';
        }
        if (status.includes('đã tiếp nhận')) {
            return 'bg-blue-100 text-blue-800 border-blue-200';
        }
        return 'bg-yellow-100 text-yellow-800 border-yellow-200';
    };

    const getEventIcon = (event: UserEvent) => {
        const status = event.status.toLowerCase();
        if (status.includes('hoàn thành')) {
            return <i className="fas fa-check-circle mr-1"></i>;
        }
        if (status.includes('đã tiếp nhận')) {
            return <i className="fas fa-tools mr-1"></i>;
        }
        return <i className="fas fa-clock mr-1"></i>;
    };
        ...
        <div className="bg-[#0f172a] min-h-screen py-8">
            <div className="container mx-auto px-4">
                ...
                <div className="mb-6 text-gray-400">
                    ...
                    <Link to="/dashboard/assignments" className="hover:text-white">
                        Lịch phân công
                    </Link>
                    <i className="fas fa-chevron-right text-xs mx-2"></i>{' '}
                ...
                <div className="bg-[#182233] border border-[#243447] rounded-lg p-6 mb-6">
                    ...
                    <div className={`grid ${viewMode === 'month' ? 'grid-cols-7' :
                        ...
                        {calendar.map((day, index) => (
                            <div
                                key={index}
                                className={`bg-[#1b2a3b] min-h-[120px] ${!day.isCurrentMonth ? 'opacity-50' : ''} ${day.date.toDateString() === new Date().toDateString() ? 'ring-2 ring-blue-500' : ''
                                    }`}
                            >
                                <div className="p-2">
                                    ...
                                    <div className="mt-2 space-y-1">
                                        {day.events.map((event) => (
                                            <button
                                                ...
                                                className={`w-full text-left px-2 py-1 rounded-md text-xs font-medium ${getEventColor(event)} hover:opacity-75 transition-opacity flex items-center`}
                                            >
                                                {getEventIcon(event)}
                ...
                <div className="bg-[#182233] border border-[#243447] rounded-lg p-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div className="flex items-center">
                            ...
                            <span className="text-gray-300">Đang chờ xử lý</span>
                        </div>
                        <div className="flex items-center">
                            <div className="w-4 h-4 bg-blue-100 rounded mr-2"></div>
                            <span className="text-gray-300">Đã tiếp nhận</span>
                        </div>
                        <div className="flex items-center">
